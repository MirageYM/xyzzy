...
int
tab_bar::notify_mousewheel (WPARAM wparam, LPARAM lparam,  LRESULT &r ) const
{

	HWND hwnd = GetDlgItem (b_hwnd, IDC_TAB_SPIN);
	DWORD range = SendMessage (hwnd, UDM_GETRANGE, 0, 0);
	int mn = LOWORD (range), mx = HIWORD (range);

	DWORD dpos = SendMessage(hwnd, UDM_GETPOS, 0, 0 );
	int pos = LOWORD( dpos );

	int zdelta = GET_WHEEL_DELTA_WPARAM(wparam);

	int clickDelta = HIWORD(wparam) / WHEEL_DELTA;
	clickDelta = zdelta / 30;
	clickDelta = -clickDelta;

	if( zdelta != 0 ){

		int newPos = pos + clickDelta;
		...
		SendMessage (hwnd, UDM_SETPOS, 0, MAKELPARAM ( newPos, 0));
		PostMessage (b_hwnd, WM_HSCROLL,
					 MAKEWPARAM (SB_THUMBPOSITION, newPos ),
					 LPARAM (hwnd));
		PostMessage (b_hwnd, WM_HSCROLL,
					 MAKEWPARAM (SB_ENDSCROLL, newPos ),
					 LPARAM (hwnd));
...
tab_bar::wndproc (UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch (msg)
	{
	 ...
	 case WM_MOUSEWHEEL:
		{
			LRESULT r;
			if (notify_mousewheel (wparam, lparam, r))
				return r;
			break;
		}
...
void dock_frame::sendMouseWheelEvent(  WPARAM wparam, LPARAM lparam, POINT& pt ){
	...
	for (int i = 0; i < EDGE_MAX; i++){
		for (dock_bar *bar = f_bars[i].head (); bar; bar = bar->next ()){
			...
			GetWindowRect ( bar->b_hwnd, &r);
			if (PtInRect (&r, pt ) ){
				bar->wndproc( WM_MOUSEWHEEL, wparam, lparam );
